...
results = []
modelno = 0
alphas = [ {'alphaAIC':alphaAIC} , { 'alphaBIC' : alphaBIC} , { 'alphaCV' : alphaCV }  , { 'alphaLarsCV' : alphaLarsCV } , {'alphaPrevModeling ' : 0.0154} ]
...
for dict_item in alphas:
    for key in dict_item:
        modelno += 1
        lasso = linear_model.Lasso(alpha = dict_item[key])
        lasso.fit(XTrain_ScaledTransformed, Y)
        Y_test_Predict = lasso.predict(XTest_ScaledTransformed)
    
        TestDatametrics =  common.regressionMetrics(yTest, Y_test_Predict)     
        ...
        plt.xlabel('Predicted Value Mean MC (Lasso Regression)')
        ...
        plt.xlabel('Predicted Value  (Lasso Regression)')
        plt.ylabel('Residual')
        plt.title(modelno)
        plt.show()
...
bestModel = linear_model.Lasso(alpha = alphaAIC)
...
for city in cities:
    ...
    plt.xlabel('Predicted Value Mean MC (Lasso Regression)')
    plt.ylabel('Actual Mean MC')
    plt.title('Lasso Regression - Best model ' + city)
    plt.show() 
    ...
    cityrank = ranking.make_scatter_plot_res_pred_runLevel(city_df, city, 'tallwood','F0-F7', '' , dirname+ '/CityWiseRanking_lasso/')
    result = ''
    result = {'ModelNo' : modelno} | {'Model' : 'Global Model - Best Model'} | {'ModelCityTest' : city } | {'CityRank': cityrank} | {'Info Criterion' : InformationCriterion } | {'Model Type' : 'Baseline with Feature transformation , scaling and alpha value selection'} | {'Alpha Value' : alphaAIC} | vars(metricsCitywise) 
...
writer = pd.ExcelWriter( dirname + '/' + 'ModelResults_Lasso.xlsx')
w1.to_excel(writer, sheet_name = 'Lasso', index=False)
w2.to_excel(writer, sheet_name = 'Lasso-CityWise', index=False)
writer.save()
