...
for n in range(1,3) :
        ...
        parameters = {'n_components':[n]}
        pls1 = PLSRegression( )
        pls = GridSearchCV(pls1, parameters, scoring = 'neg_mean_squared_error', cv=10, n_jobs=6)    
        pls.fit(X_PLS , Y_PLS)
        ...
        loadings = ''
        loadings = pls.best_estimator_.coef_
        ...
        for counter in  range(len(plsRankRowSorted)):
            ...
            data = {'PLSComponents' : [n],
                        'FeatureList'    : [features] ,
                        'NoofFeatures' : [i + 1]}    
            dfpls = dfpls.append(data , ignore_index=True)
            
            
# Model with the data in dfpls
FeatureSelectionReports = []
PLSResultsTable = pd.DataFrame()
for row in dfpls.iterrows():
    FeatureSelectionReport = datamodels.FeatureSelectionReport('pls','' , 0, [],pd.DataFrame(),pd.DataFrame() ,'')  
    X_train_selected = pd.DataFrame()
    X_test_selected = pd.DataFrame()
    
    X_train_selected = XTrain_Scaled[row[1]['FeatureList'][0]]
    X_test_selected =  XTest_Scaled[row[1]['FeatureList'][0]]  
    print('#####')
    print( row[1]['PLSComponents'][0])
    regressor = PLSRegression(n_components = row[1]['NoofFeatures'][0] )
    
    parameters2 = {'n_components':[row[1]['NoofFeatures'][0]]}
    pls2 = PLSRegression( )
    regressor = GridSearchCV(pls2, parameters2, scoring = 'neg_mean_squared_error', cv=10, n_jobs=6)    
    regressor.fit(X_PLS , Y_PLS)
    
    
         
    FeatureSelectionReport.noofFeatures =    row[1]['NoofFeatures'][0]                  
    FeatureSelectionReport.selectedFeatures =   row[1]['FeatureList'][0]       
    ...
    regressor.fit(X_train_selected , Y_PLS )
    ypred_Train = regressor.predict(X_train_selected)
    # check residual
    ...
    ypred_Test = regressor.predict(X_test_selected )   
    
    # check residual
    plt.scatter( x = ypred_Test , y = Y_TestPLS)
    plt.xlabel('Predicted Value Mean MC (PLS Regression)')
    plt.ylabel('Actual Mean MC')
    plt.title(modelno)
    plt.show() 
    
    residual = np.array(Y_TestPLS).reshape(-1,1) - ypred_Test
    sns.regplot(y = residual, x = ypred_Test)
    plt.xlabel('Predicted Value  (PLS Regression)')
    plt.ylabel('Residual')
    plt.title(modelno)     
    plt.show()
            
    FeatureSelectionReports.append(FeatureSelectionReport)
    TestDatametrics = common.regressionMetrics(((np.array(Y_TestPLS).reshape(-1,1))) , ypred_Test )
    data = [{ 'ModelNo':modelno, 'No Of Components' : row[1]['PLSComponents'][0] , 'noofFeatures' : row[1]['NoofFeatures'][0] , 'RMSE' : TestDatametrics.RMSE   }]
    PLSResultsTable = PLSResultsTable.append(data)
 
    result = {'ModelNo' : modelno} | {'Model' : 'Global Model'} | {'Model Type' : 'Baseline with Feature transformation ,scaling and selection'} | {'FeatSelection' : FeatureSelectionReport.featureSelection} | {'No Of Components' : row[1]['PLSComponents'][0]} | vars(FeatureSelectionReport) | {'RMSE' : TestDatametrics.RMSE }  | vars(TestDatametrics)  
...
cityresults = []
modelno = 304
bestModel =  FeatureSelectionReports[modelno - 1]
...
for city in cities:
    ...
    cityrank = ranking.make_scatter_plot_res_pred_runLevel(city_df, city, 'tallwood','F0-F7', '' , dirname + '/CityWiseRanking_PLS/')
    InformationCriterion = common.logLiklihood(Yactual_test_city  , ypred_test_city  , Xtest_city.shape[1] )
    result = ''
    result = {'ModelNo' : modelno} | {'Model' : 'Global Model - Best Model'} | {'ModelCityTest' : city } | {'CityRank': cityrank} |{'Model Type' : 'Baseline with Feature selection'} |  vars(metricsCitywise)
...
writer = pd.ExcelWriter( dirname + '/' + 'ModelResults_PLS.xlsx')
w1.to_excel(writer, sheet_name = 'PLS', index=False)
w2.to_excel(writer, sheet_name = 'PLS-CityWise', index=False)
writer.save()
#%% 
