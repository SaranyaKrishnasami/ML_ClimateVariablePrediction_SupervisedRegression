...
params = [ {'bootstrap': True, 'max_depth': 10, 'max_features': 'auto', 'min_samples_leaf': 1, 'min_samples_split': 2, 'n_estimators': 20} ]
...
bestModel = GradientBoostingRegressor(learning_rate= 0.1, max_depth= 10, max_features= 'auto', min_samples_leaf= 2, min_samples_split= 2, n_estimators= 48, random_state= 10, subsample= 0.8 )
...
for city in cities:
    ...
    plt.xlabel('Predicted Value Mean MC (XGBoost)')
    plt.ylabel('Actual Mean MC')
    plt.title('XGBoost - Best model ' + city)
    plt.show() 
    
    residual = s2 - s3
    sns.regplot(y = residual, x = s3)
    plt.xlabel('Predicted Value  (XGBoost)')
    plt.ylabel('Residual')
    plt.title('XGBoost - Best model ' + city)
    plt.show()
    
    InformationCriterion = common.logLiklihood(Yactual_test_city  , ypred_test_city  , Xtest_city.shape[1] )
    cityrank = ranking.make_scatter_plot_res_pred_runLevel(city_df, city, 'tallwood','F0-F7', '' , dirname+ '/CityWiseRanking_XGBoost/')
    result = ''
...
writer = pd.ExcelWriter( dirname + '/' + 'ModelResults_XGBoost.xlsx')
w1.to_excel(writer, sheet_name = 'XGBoost', index=False)
w2.to_excel(writer, sheet_name = 'XGBoost-CityWise', index=False)
writer.save()
