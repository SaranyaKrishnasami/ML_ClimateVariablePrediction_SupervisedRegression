...
bestModel = RandomForestRegressor(bootstrap = True, max_depth = 10, max_features = 'auto', min_samples_leaf = 1, min_samples_split = 2, n_estimators = 20)
...
for city in cities:
    ...
    plt.xlabel('Predicted Value Mean MC (Random Forest)')
    plt.ylabel('Actual Mean MC')
    plt.title('Random Forest - Best model ' + city)
    plt.show() 
    
    residual = s2 - s3
    sns.regplot(y = residual, x = s3)
    plt.xlabel('Predicted Value  (Random Forest)')
    plt.ylabel('Residual')
    plt.title('Random Forest - Best model ' + city)
    plt.show()
    
    InformationCriterion = common.logLiklihood(Yactual_test_city  , ypred_test_city  , Xtest_city.shape[1] )
    cityrank = ranking.make_scatter_plot_res_pred_runLevel(city_df, city, 'tallwood','F0-F7', '' , dirname+ '/CityWiseRanking_RF/')
    result = ''
...
writer = pd.ExcelWriter( dirname + '/' + 'ModelResults_RF.xlsx')
w1.to_excel(writer, sheet_name = 'RF', index=False)
w2.to_excel(writer, sheet_name = 'RF-CityWise', index=False)
writer.save()
